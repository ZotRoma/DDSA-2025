...
async def download(torrent_path: str):
    ...
    reliable_peers = set()  # Множество для хранения надежных пиров
    failed_pieces = set()

    async def fetch_piece(idx):
        piece_size = pm.get_piece_size(idx)  # Получаем точный размер куска
        buffer = bytearray(piece_size)
        max_attempts = 5  # Максимальное количество попыток для одного куска
        for attempt in range(max_attempts):
            # Выбираем пир: сначала из надежных, если есть, иначе случайный
            if reliable_peers:
                ip, port = random.choice(list(reliable_peers))
                logger.debug(f"Using reliable peer {ip}:{port} for piece {idx}")
            else:
                ip, port = random.choice(peers)
                logger.debug(f"Using random peer {ip}:{port} for piece {idx}")
            ...
            try:
                ...
                if pm.verify(idx, buffer):
                    ...
                    reliable_peers.add((ip, port))  # Добавляем пир в надежные
                    logger.info(f"Piece {idx} OK from {ip}:{port}")
                    return True
                else:
                    logger.warning(f"Piece {idx} hash mismatch from {ip}:{port}, will retry")
                    if (ip, port) in reliable_peers:
                        reliable_peers.remove((ip, port))  # Удаляем из надежных, если хэш не совпал
            except Exception as e:
                logger.error(f"Error piece {idx} from {ip}:{port} -> {e}")
                if (ip, port) in reliable_peers:
                    reliable_peers.remove((ip, port))  # Удаляем из надежных, если произошла ошибка
            await asyncio.sleep(2 ** attempt)  # Экспоненциальная задержка перед следующей попыткой
        logger.error(f"Failed to download piece {idx} after {max_attempts} attempts")
        return False
...
if __name__ == '__main__':
    #torrent_path = '[AI] Фостер Дэвид - Генеративное глубокое обучение. Как не мы рисуем картины, пишем романы и музыку. 2-е межд изд. [2024, PDF, RUS [rutracker-6511606].torrent'
    torrent_path = 'Грей Джо - Социальная инженерия и этичный хакинг на практике [2023, PDF, RUS] [rutracker-6691768].torrent'
    asyncio.run(download(torrent_path))